export const words = [
  { text: "Ideas", imgPath: "/images/ideas.svg" },
  { text: "Concepts", imgPath: "/images/concepts.svg" },
  { text: "Designs", imgPath: "/images/designs.svg" },
  { text: "Code", imgPath: "/images/code.svg" },
  { text: "Ideas", imgPath: "/images/ideas.svg" },
  { text: "Concepts", imgPath: "/images/concepts.svg" },
  { text: "Designs", imgPath: "/images/designs.svg" },
  { text: "Code", imgPath: "/images/code.svg" },

];
export const counterItems = [
  { 
    value: 1, 
    suffix: "", 
    label: "Internship – Web Development (React + Express)",
    prefix: ""
  },
  { 
    value: 3, 
    suffix: "rd Year", 
    label: "B.Tech (Data Science)",
    prefix: ""
  },
  { 
    value: 15, 
    suffix: "+", 
    label: "Repositories",
    prefix: ""
  },
  { 
    value: 9, 
    suffix: "+", 
    label: "Certifications",
    prefix: ""
  },
];
export const navLinks = [
  {
    name: "Projects",
    link: "#work",
  },
  {
    name: "Experience",
    link: "#experience",
  },
  {
    name: "Skills",
    link: "#skills",
  },
  {
    name: "Certifications",
    link: "#certificates",
  },
];
export const logoIconsList = [
  {
    imgPath: "/images/logos/company-logo-1.png",
  },
  {
    imgPath: "/images/logos/company-logo-2.png",
  },
  {
    imgPath: "/images/logos/company-logo-3.png",
  },
  {
    imgPath: "/images/logos/company-logo-4.png",
  },
  {
    imgPath: "/images/logos/company-logo-5.png",
  },
  {
    imgPath: "/images/logos/company-logo-6.png",
  },
  {
    imgPath: "/images/logos/company-logo-7.png",
  },
  {
    imgPath: "/images/logos/company-logo-8.png",
  },
  {
    imgPath: "/images/logos/company-logo-9.png",
  },
  {
    imgPath: "/images/logos/company-logo-10.png",
  },
  {
    imgPath: "/images/logos/company-logo-11.png",
  },
];

export const experienceData = [
  {
    title: "Web Development Intern",
    company: "DECRYPTON LLC",
    date: "May 2025 - Aug 2025",
    type: "Internship",
    location: "Remote",
    description: "Built a Hyperlocal Delivery Application with features like product browsing, cart management, user authentication, and real-time order placement. Utilized React.js, Node.js, Express.js, MongoDB, Redux, Tailwind CSS, and Vite to develop a responsive, full-stack application.",
    technologies: ["HTML", "CSS", "React.js", "Node.js", "Express.js", "MongoDB", "Redux", "Tailwind CSS", "Vite"],
    logoPath: "/images/logos/company-logo-3.png",
  },
  {
    title: "Photography Co Lead",
    company: "Computer Society of India (CSI) - SRM NCR",
    date: "Oct 2024 - Present",
    type: "Part-time",
    location: "Delhi, India",
    description: "Leading photography and video editing initiatives for club events. Managing visual content creation and documentation for various CSI activities and programs.",
    technologies: ["Video Capture", "Video Editing", "Photography", "Content Creation"],
    logoPath: "/images/logos/company-logo-1.png",
  },
  {
    title: "Video Editor",
    company: "National Service Scheme",
    date: "Sep 2024 - Present",
    type: "Part-time",
    location: "Ghaziabad, Uttar Pradesh, India",
    description: "Creating and editing video content for social service initiatives. Producing engaging visual materials to promote community service activities and events.",
    technologies: ["Video Capture", "Social Services", "Video Editing", "Content Creation"],
    logoPath: "/images/logos/company-logo-2.png",
  },
];

export const skillsData = [
  {
    name: "JavaScript",
    icon: "/images/logos/react.png",
    level: "Advanced",
  },
  {
    name: "Java",
    icon: "/images/logos/company-logo-1.png",
    level: "Advanced",
  },
  {
    name: "C++",
    icon: "/images/logos/company-logo-2.png",
    level: "Advanced",
  },
  {
    name: "React.js",
    icon: "/images/logos/react.png",
    level: "Advanced",
  },
  {
    name: "Node.js",
    icon: "/images/logos/node.png",
    level: "Advanced",
  },
  {
    name: "Express.js",
    icon: "/images/logos/node.png",
    level: "Advanced",
  },
  {
    name: "MongoDB",
    icon: "/images/logos/company-logo-1.png",
    level: "Advanced",
  },
  {
    name: "Redux",
    icon: "/images/logos/react.png",
    level: "Intermediate",
  },
  {
    name: "Tailwind CSS",
    icon: "/images/logos/company-logo-3.png",
    level: "Advanced",
  },
  {
    name: "HTML/CSS",
    icon: "/images/logos/company-logo-4.png",
    level: "Advanced",
  },
  {
    name: "Vite",
    icon: "/images/logos/company-logo-5.png",
    level: "Advanced",
  },
  {
    name: "Git & GitHub",
    icon: "/images/logos/git.svg",
    level: "Advanced",
  },
];

export const testimonialsData = [
  {
    name: "Client Name",
    role: "Project Manager",
    review: "Tanmay delivered an exceptional full-stack application that exceeded our expectations. His attention to detail and technical expertise made the project a success.",
    image: "/images/client1.png",
    rating: 5,
  },
  {
    name: "Client Name",
    role: "CEO",
    review: "Working with Tanmay was a great experience. He transformed our ideas into a beautiful, functional web application. Highly recommended!",
    image: "/images/client2.png",
    rating: 5,
  },
  {
    name: "Client Name",
    role: "Founder",
    review: "Tanmay's technical skills and professionalism are outstanding. He delivered our project on time and with excellent quality.",
    image: "/images/client3.png",
    rating: 5,
  },
];

export const socialLinks = [
  {
    name: "LinkedIn",
    url: "https://linkedin.com/in/tanmay--kapoor",
    icon: "/images/linkedin.png",
  },
  {
    name: "GitHub",
    url: "https://github.com/Tanmayk03",
    icon: "/images/logos/git.svg",
  },
  {
    name: "Email",
    url: "mailto:tanmaykapoor003@gmail.com",
    icon: "/images/insta.png",
  },
];